...
class Akismet
{
  ...
  public static function schedule_approval_fallback($id, $comment)
  {
    ...
    if ($approval_delay) {
      ...
      self::log(
        "Scheduled approval fallback for " . (time() + $approval_delay) . " for comment #" . $id,
      );
    } else {
    ...
    if (wp_get_comment_status($comment_id) == "unapproved") {
      if (self::last_comment_status_change_came_from_akismet($comment_id)) {
        ...
        if (!$comment) {
          ...
          self::log(
            "Not approving comment #" . $comment_id . " because it does not pass check_comment()",
          );
        }
      ...
      self::log(
        "No need to fallback approve comment #" . $comment_id . " because it is not pending.",
      );
    }
  }
  ...
  public static function delete_old_comments()
  {
    ...
    while (
      ...
      $wpdb->query(
        $wpdb->prepare(
          "DELETE FROM {$wpdb->commentmeta} WHERE comment_id IN ( " . $format_string . " )",
          $comment_ids,
        ),
      );
...
<p><?php echo esc_html($message); ?></p>
  ...
  {
    if (
      in_array($tag, ["contact-form", "gravityform", "contact-form-7", "formidable", "fluentform"])
    ) {
      self::load_form_js();
    }
    ...
    if ($history) {
      ...
      foreach ($history as $akismet_history_entry) {
        ...
        if (
          ...
          in_array(
            $akismet_history_entry["event"],
            ["recheck-ham", "check-ham", "cron-retry-ham", "webhook-ham", "webhook-ham-noaction"],
            true,
          )
        ) {
          return "false";
