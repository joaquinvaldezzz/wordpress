...
class Akismet_Admin
{
        ...
        if (!Akismet::predefined_api_key()) {
          $current_screen->add_help_tab([
            ...
            "content" =>
              ...
              "</strong> - " .
              esc_html__("The subscription status - active, cancelled or suspended", "akismet") .
              "</p>",
          ]);
        }
      }
  ...
  public static function save_key($api_key)
  {
    ...
    if ($key_status == "valid") {
      ...
      if ($akismet_user) {
        if (in_array($akismet_user->status, [Akismet::USER_STATUS_ACTIVE, "active-dunning"])) {
    ...
    echo "<p>" .
      sprintf(
        ...
        esc_url(
          add_query_arg(
            ["page" => "akismet-admin"],
            admin_url(isset($submenu["edit-comments.php"]) ? "edit-comments.php" : "edit.php"),
          ),
        ),
  ...
  public static function rightnow_stats()
  {
    ...
    if ($queue_count = self::get_spam_count()) {
      ...
      $queue_text = sprintf(
        __("There&#8217;s nothing in your <a href='%s'>spam queue</a> at the moment.", "akismet"),
        esc_url($link),
      );
    }
      ...
      foreach ($history as $row) {
        ...
        if (!empty($row["message"])) {
          ...
          switch ($row["event"]) {
            case "recheck-spam":
              $message = esc_html(
                __("Akismet re-checked and caught this comment as spam.", "akismet"),
              );
              break;
            case "check-spam":
            ...
            case "report-spam":
              if (isset($row["user"])) {
                ...
                $message = esc_html(
                  sprintf(__("%s reported this comment as spam.", "akismet"), $row["user"]),
                );
              } elseif (!$message) {
            ...
            case "report-ham":
              if (isset($row["user"])) {
                /* translators: The placeholder is a username. */
                $message = esc_html(
                  sprintf(__("%s reported this comment as not spam.", "akismet"), $row["user"]),
                );
              } elseif (!$message) {
            ...
            case "cron-retry-spam":
              $message = esc_html(
                __("Akismet caught this comment as spam during an automatic retry.", "akismet"),
              );
              break;
            case "cron-retry-ham":
              $message = esc_html(
                __("Akismet cleared this comment during an automatic retry.", "akismet"),
              );
              break;
            case "check-error":
            ...
            case "recheck-error":
              if (isset($row["meta"], $row["meta"]["response"])) {
                ...
                $message = sprintf(
                  esc_html(
                    __("Akismet was unable to recheck this comment (response: %s).", "akismet"),
                  ),
                  "<code>" . esc_html($row["meta"]["response"]) . "</code>",
                );
              } else {
            ...
            case "webhook-ham":
              $message = esc_html(
                __("Akismet cleared this comment and updated its status via webhook.", "akismet"),
              );
              break;
            ...
            default:
              if (preg_match("/^status-changed/", $row["event"])) {
                ...
                if (isset($row["user"])) {
                  ...
                  $message = sprintf(
                    esc_html(__('%1$s changed the comment status to %2$s.', "akismet")),
                    $row["user"],
                    "<code>" . esc_html($new_status) . "</code>",
                  );
                }
        ...
        if (!empty($message)) {
          ...
          if (isset($row["time"])) {
            ...
            $time_html =
              ...
              esc_attr($time) .
              '">' .
              sprintf(esc_html__("%s ago", "akismet"), human_time_diff($row["time"])) .
              "</span>";
